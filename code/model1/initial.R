...
times = 50;               #how many times

# YOU DO NOT NEED TO CHANGE ANYTHING BELOW HERE 
# Function Definitions
moveP3 = function(p,sites, dt = .3, directionChange = .2,  
...
#graph the picture
 ...
 ilist = 0;
for (i in 1:times){
    pm = t(apply(pm, 1,function(pin){moveP3(p = pin, sites = Xsite, maxX = 200);}))
    ilist = c(ilist,i)
    ...
    plot(pm[,1], pm[,2], col = "blue", xlim = c(0,maxX), ylim = c(0,100),
            xlab = "", ylab = ""); 
    plot( ilist, clist, 
          xlim = c(0,nmax), ylim = c(0,numParticles),

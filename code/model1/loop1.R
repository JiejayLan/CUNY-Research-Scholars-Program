...
times = 1;              # how many times for one experiment
runs=1;                   # how many experiments runs
maxX=500;                 # width
maxY=500;                 # height
...
for (i in 1:times){
 ...
 #find r^2
  for (i in 1:times){
    z=0;
    z=numParticles*(1- exp(-kest*i));
    klist=c(klist,z);
}
 t_line=lm(y~klist);
 r=summary(t_line)$r.squared;

 k=c(k,kest);  #store K value
}

write.table(k, "clipboard", sep="\t", row.names=FALSE);   #output into excel

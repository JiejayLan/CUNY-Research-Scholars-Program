...
runs = 1;
heights = seq(51, 51, by = 10);  #seq(40, 60, by = 10); 
heights;
numParticles = 3000;              # how many particles in box
its = 1000;                      
...
for(n in 0:2026){
  k=2*n+1;
  sum=sum+8*numParticles/((k^2)*(pi^2))*exp(-(k^2*pi^2*0.5*1/(0.5*(heights-1))^2));
 }
PDE=numParticles-sum;
for(i in 2:its){
sum=0;
 for(n in 0:2026){
  k=2*n+1;
  sum=sum+8*numParticles/((k^2)*(pi^2))*exp(-(k^2*pi^2*0.5*i/(0.5*(heights-1))^2));
 }
  PDE=rbind(PDE,numParticles-sum);
}
...
#-------- draw legend
legend(its/10,numParticles/1.5,
  c("data","regression", "theory","PDE"),
  lty = c(NA_integer_,1,1),
  lwd = c(3,3,3,3),
  pch = c(1, NA_integer_,NA_integer_),
  col=c("red","blue","cyan","BLACK"),
  bty = "n" );
#--------------------------

...
################################################################ not used
#----------------------------------------------------------
# summary(mCondensationFirstOrd)  # uncomment if you want reg details
#---------------------------------------------------------
# kreg = mCondensationFirstOrd$m$getPars() # get regression k estimate and then plot it
# text(0*its, 1*numParticles, pos = 4,paste("regression k = ", toString(round(kreg,6)) )); 
# -------------------------------------------------------------
# plot theory k value
#text(.4*its, .55*numParticles, pos = 4,paste("theory k = ", toString(round(kt,6)) )); 
#############################################################################
#-------- draw legend
legend(.4*its,.45*numParticles,
  c("data (no mixing)","ODE (k from regression)", "PDE (k = 1/4)"),
  lty = c(NA_integer_,1,1),
  lwd = c(3,3,6),
  pch = c(1, NA_integer_,NA_integer_),
  col=c("red", "black" ,"cyan"),
  bty = "n" );
#--------------------------
